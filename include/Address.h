...
public:
	...
	explicit CAddress(unsigned short port, SocketType type = SOCKET_TYPE_TCP)
	{
		memset(&m_Addr, 0, sizeof(m_Addr));
		m_Addr.sin_family = AF_INET;
		m_Addr.sin_port = SocketOpt::Htons(port);
		m_Addr.sin_addr.s_addr = INADDR_ANY;
		m_Type = type;
	}

	CAddress(const char* addr, unsigned short port, SocketType type = SOCKET_TYPE_TCP)
	{
		memset(&m_Addr, 0, sizeof(m_Addr));
	...
	CAddress(const struct sockaddr_in& addr, SocketType type = SOCKET_TYPE_TCP)
	{
		m_Addr = addr;
		m_Type = type;
	}

	CAddress(unsigned long ip, SocketType type = SOCKET_TYPE_TCP)
	{
