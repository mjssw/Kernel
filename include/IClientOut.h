...
public:
	...
	/// @param port 端口
	/// @param type 套接字协议类型
	virtual void Connect(uint64_t sessionID, const char* ip, unsigned short port, SocketType type = SOCKET_TYPE_TCP) = 0;
	...
	/// @param type 套接字协议类型
	virtual void Connect(uint64_t sessionID, const Kernel::CAddress* serverAddr) = 0;
	...
	virtual int SyncConnect(uint64_t sessionID, const char* addr, int port, bool isUserCall = true, bool isAddrClientMgr = true) = 0;
	...
	/// @param type 套接字类型
	/// @return 返回结果，成功返回true，失败返回false
	virtual bool RequestP2P(uint64_t srcSessionID, uint64_t srcUserID, uint64_t desUserID, const char* helpHoldIP, unsigned short helpHoldPort, SocketType helpHoldType = SOCKET_TYPE_TCP) = 0;
